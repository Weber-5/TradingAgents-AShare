        ...
        return (
            <button
                ...
                className="fixed right-0 top-1/2 -translate-y-1/2 z-30
                           ...
                           border border-r-0 border-slate-200 dark:border-slate-700 rounded-l-lg
            ...
            >
                ...
                <ChevronLeft className="w-3 h-3 text-slate-400 dark:text-slate-500" />
            </button>
        )
    }
    ...
    if (!isPanelExpanded) {
        return (
            ...
            >
                ...
                {hasData ? (
                    <>
                        <span className="text-sm font-mono text-slate-900 dark:text-white tabular-nums">
                            {fmtTokens(grandTotalTokens)}
                        </span>
                        {hasPricing && totalCost > 0 && (
                            <span className="text-xs font-mono text-amber-600 dark:text-amber-400 tabular-nums">
                                {fmtCost(totalCost, currency)}
                            </span>
                        )}
                    </>
                ) : (
                    <span className="text-xs text-slate-500 dark:text-slate-400">Token 用量</span>
                )}
    ...
    return (
        <div
            className="fixed bottom-5 right-5 z-30
                       ...
                       rounded-xl shadow-xl shadow-black/5 dark:shadow-2xl dark:shadow-black/50
                       w-[360px] max-h-[520px]
                       flex flex-col
                       animate-in slide-in-from-bottom duration-200"
        >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 shrink-0">
                <div className="flex items-center gap-2">
                    <Coins className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                    <span className="text-sm font-semibold text-slate-900 dark:text-white">Token 用量</span>
                </div>
            ...
            <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-800 shrink-0">
                <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500 dark:text-slate-400">
                        总计 <span className="font-mono text-slate-900 dark:text-white tabular-nums">{fmtTokens(grandTotalTokens)}</span> tokens
                    </span>
                    {hasPricing && (
                ...
                <div className="mt-1.5 flex gap-0.5 h-1.5 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                    {grandTotalTokens > 0 && (
                        <>
                            <div
                ...
                <div className="flex items-center gap-4 mt-1 text-[10px] text-slate-400 dark:text-slate-500">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2">
                {agentList.map((agent) => {
                    ...
                    return (
                        <div key={agent.agentName} className="space-y-1">
                            ...
                            <div className="flex gap-0.5 h-1 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                                {agent.totalTokens > 0 && (
                                    <>
                                        <div
                ...
                {!hasData && (
                    <div className="text-xs text-slate-400 dark:text-slate-500 text-center py-6">
                        暂无 Token 数据
                        <br />
                        <span className="text-slate-400 dark:text-slate-600">启动一次分析后将在此显示用量</span>
                    </div>
                )}
            </div>
            ...
            {!hasPricing && hasData && (
                <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-800 shrink-0">
                    <button
                        onClick={() => setConfigModalOpen(true)}
                        className="w-full text-xs text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors py-1"
                    >
                        配置模型价格以显示费用估算 →
                    </button>
                </div>
            ...
            {hasPricing && hasData && (
                <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-800 shrink-0 flex items-center justify-between text-[10px] text-slate-400 dark:text-slate-500">
